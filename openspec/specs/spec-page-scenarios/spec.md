...
`generateSpecPage()` SHALL remove `## ADDED Requirements`, `## MODIFIED Requirements`, and `## REMOVED Requirements` headings from the generated page content. Requirements under these sections SHALL be preserved — only the section heading line itself is removed.
...
`generateSpecPage()` SHALL transform each `#### Scenario: <title>` block into a VitePress `:::details <title>` container, so that scenarios are collapsible and visually distinct from requirement text.
...
#### Scenario: Scenario block ends at the next heading or end of file
